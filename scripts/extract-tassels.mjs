 ...
 * Reads assets/painting/tree-cutout*.webp (untouched sources), writes public/images/tree-cutout*.webp
 * (ends painted out) and public/images/tassel*.png (the ends alone, transparent elsewhere).
 * The band and the knot stay in the cut-out: a thread tied round a trunk does not slide.
 */
...
async function run(variant) {
  ...
  // Paint the ends out of the cut-out: each masked pixel takes the mean of its nearest unmasked row neighbours.
  ...
  for (let y = REGION.y; y < REGION.y + REGION.h; y++)
    for (let x = REGION.x; x < REGION.x + REGION.w; x++) {
      ...
      const o = idx(x, y);
      for (let c = 0; c < 4; c++) out[o + c] = Math.round(picks.reduce((s, p) => s + data[p + c], 0) / picks.length);
    }
