 ...
 *   public/images/thread-band*.png   the band and the knot round the trunk (they never move)
 *   public/images/tassel*.png        the two loose ends below the knot (they swing)
...
const FRAMES = {
  wide: { name: "tree-cutout", BAND: { x: 1290, y: 722, w: 160, h: 56 }, ENDS: { x: 1330, y: 778, w: 150, h: 102 }, smooth: false, bandFillMode: "nearest", bandDirs: [[1, 0], [-1, 0], [0, 1], [0, -1]], endsDirs: [[0, 1], [0, -1]] },
...
async function run(frame, variant) {
  ...
  // theirs is the thread alone.
  const crop = async (R, file) => {
    const out = Buffer.alloc(R.w * R.h * 4);
    for (let y = 0; y < R.h; y++)
      for (let x = 0; x < R.w; x++) {
        const i = idx(R.x + x, R.y + y), o = (y * R.w + x) * 4;
        for (let c = 0; c < 4; c++) out[o + c] = data[i + c];
      }
    await sharp(out, { raw: { width: R.w, height: R.h, channels: 4 } }).png().toFile(file);
  };
  await crop(BAND, `public/images/${base.replace("tree-cutout", "thread-band")}${suffix}.png`);
